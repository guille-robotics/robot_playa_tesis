...
class PowerCalculator(Node):
    ...
    def update_battery_bar(self):
        # Asume que el voltaje máximo de la batería es 24V
        max_voltage = 24.0
        # Calcula el porcentaje de batería restante
        battery_percentage = min(max(self.voltage / max_voltage, 0), 1)
        battery_canvas.coords(battery_fill, 0, 0, battery_percentage * 200, 50)
...
if __name__ == '__main__':
    ...
    # Crear la barra de batería
    Label(root, text="Battery Status:").grid(row=5, column=0)
    battery_canvas = Canvas(root, width=200, height=50, bg='white', bd=1, relief='solid')
    battery_canvas.grid(row=5, column=1)
    battery_fill = battery_canvas.create_rectangle(0, 0, 0, 50, fill='green')
